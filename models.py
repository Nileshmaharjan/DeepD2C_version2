...
class BuildModel:
    ...
    def __call__(self, encoder, decoder, discriminator, secret_input, image_input, l2_edge_gain, borders, secret_size,
        ...
        summary_op = tf.compat.v1.summary.merge([
                                                    ...
                                                    tf.compat.v1.summary.scalar('image_loss', image_loss_op,
                                                                                family='train'),
                                                    tf.compat.v1.summary.scalar('G_loss', G_loss, family='train'),
                                                    tf.compat.v1.summary.scalar('secret_loss', secret_loss_op,
                                                                                family='train'),
                                                    tf.compat.v1.summary.scalar('dis_loss', D_loss, family='train'),
                                                    tf.compat.v1.summary.scalar('Y_loss', yuv_loss_op[0],
                                                                                family='color_loss'),
                                                    tf.compat.v1.summary.scalar('U_loss', yuv_loss_op[1],
                                                                                family='color_loss'),
                                                    tf.compat.v1.summary.scalar('V_loss', yuv_loss_op[2],
                                                                                family='color_loss'),
                                                ] + transform_summaries)
...
#         with  tf.compat.v1.variable_scope("acc"):
#             secret_pred = tf.round(tf.sigmoid(secret_pred))
#             correct_pred = tf.compat.v1.to_int64(tf.shape(secret_pred)[1]) - tf.compat.v1.count_nonzero(secret_pred - secret_true, axis=1)
#
#             str_acc = 1.0 - tf.compat.v1.count_nonzero(correct_pred - tf.compat.v1.to_int64(tf.shape(secret_pred)[1])) / tf.size(
#                 correct_pred,
#                 out_type=tf.int64)
#
...
#
#         # Color transformation
#         contrast_scale = tf.random.uniform(shape=[tf.shape(encoded_image)[0]], minval=contrast_params[0],
#                                            maxval=contrast_params[1])
